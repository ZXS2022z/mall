...
import { getCartListAPI, addCartAPI, deleteCartAPI, updateCartAPI} from "@/api/cart";
...
export const useCartStore = defineStore('cart', () => {
  ...
  const totalPrice = computed(() => cartList.value.reduce((acc,cur) => acc + cur.price * cur.count,0).toFixed(2))

  const selectedCount = computed(() => cartList.value.filter(item => item.selected).reduce((acc,cur) => acc + cur.count,0))
  const selectedPrice = computed(() => cartList.value.filter(item => item.selected).reduce((acc,cur) => acc + cur.price * cur.count,0).toFixed(2))
